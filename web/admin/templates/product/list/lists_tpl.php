            ...
            <table class="table table-hover">
                <thead>
                    <tr>
						...
						<?php if(isset($config['product'][$type]['show_images_list']) && $config['product'][$type]['show_images_list'] == true) { ?>
							<th class="align-middle">Hình</th>
						<?php } ?>
						<th class="align-middle" style="width:30%">Tiêu đề</th>
						<?php if(isset($config['product'][$type]['gallery_list']) && count($config['product'][$type]['gallery_list']) > 0) { ?>
                            <th class="align-middle">Gallery</th>
                        <?php } ?>
						<?php if(isset($config['product'][$type]['check_list'])) { foreach($config['product'][$type]['check_list'] as $key => $value) { ?>
                            ...
                            <tr>
                                ...
                                <?php if(isset($config['product'][$type]['check_list'])) { foreach($config['product'][$type]['check_list'] as $key => $value) { ?>
								  	<td class="align-middle text-center">
	                                	<div class="custom-control custom-checkbox my-checkbox">
	                                        <input type="checkbox" class="custom-control-input show-checkbox" id="show-checkbox-<?=$key?>-<?=$items[$i]['id']?>" data-table="product_list" data-id="<?=$items[$i]['id']?>" data-loai="<?=$key?>" <?=($items[$i][$key])?'checked':''?>>
