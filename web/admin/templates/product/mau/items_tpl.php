<?php 
	...
	$linkAdd = "index.php?com=product&act=add_mau&type=".$type."&p=".$curPage;
    $linkEdit = "index.php?com=product&act=edit_mau&type=".$type."&p=".$curPage;
    $linkDelete = "index.php?com=product&act=delete_mau&type=".$type."&p=".$curPage;
?>
<!-- Content Header -->
    ...
    <div class="container-fluid">
        <div class="row">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active">Quản lý màu sắc <?=$config['product'][$type]['title_main']?></li>
            </ol>
        </div>
    </div>
</section>
...
<section class="content">
    ...
    <div class="card card-primary card-outline text-sm mb-0">
        <div class="card-header">
            <h3 class="card-title">Danh sách màu sắc <?=$config['product'][$type]['title_main']?></h3>
        </div>
                    ...
                    <tbody>
                        <?php for($i=0;$i<count($items);$i++) { ?>
                            <tr>
                                ...
                                <td class="align-middle">
                                    <input type="number" class="form-control form-control-mini m-auto update-stt" min="0" value="<?=$items[$i]['stt']?>" data-id="<?=$items[$i]['id']?>" data-table="product_mau">
                                </td>
                                <?php if(isset($config['product'][$type]['mau_images']) && $config['product'][$type]['mau_images'] == true) { ?>
                                    <td class="align-middle">
								...
								<td class="align-middle text-center">
                                	<div class="custom-control custom-checkbox my-checkbox">
                                        <input type="checkbox" class="custom-control-input show-checkbox" id="show-checkbox-<?=$items[$i]['id']?>" data-table="product_mau" data-id="<?=$items[$i]['id']?>" data-loai="hienthi" <?=($items[$i]['hienthi'])?'checked':''?>>
