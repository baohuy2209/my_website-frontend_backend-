    ...
    <div class="container-fluid">
        <div class="row">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active">Quản lý kích thước <?=$config['product'][$type]['title_main']?></li>
            </ol>
        </div>
    </div>
</section>
...
<section class="content">
    ...
    <div class="card card-primary card-outline text-sm mb-0">
        <div class="card-header">
            <h3 class="card-title">Danh sách kích thước <?=$config['product'][$type]['title_main']?></h3>
        </div>
                        ...
                        <?php for($i=0;$i<count($items);$i++) { ?>
                            <tr>
								...
								<td class="align-middle text-center">
                                	<div class="custom-control custom-checkbox my-checkbox">
                                        <input type="checkbox" class="custom-control-input show-checkbox" id="show-checkbox-<?=$items[$i]['id']?>" data-table="product_size" data-id="<?=$items[$i]['id']?>" data-loai="hienthi" <?=($items[$i]['hienthi'])?'checked':''?>>
