...
export class CollapsibleComponent extends React.Component {
  componentDidMount() {
    ...
    for (i = 0; i < acc.length; i++) {
      acc[i].addEventListener("click", function (e) {
        e.preventDefault();
        // Highlist the button - add/remove "active-accordion" class
        this.classList.toggle("active-accordion");
        let panel;
        if (
          this.dataset.showContentAboveButton === "true" &&
  ...
  render() {
    return <div id={"rcc" + this.props.name}>{this.props.children}</div>;
  }
}
